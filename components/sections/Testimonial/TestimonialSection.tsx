...
type TestimonialRow = {
  rating: number | null;
  message: string | null;
  profiles: TestimonialProfile | null;
};
...
// دیتای مجازی (Fallback)
const fallbackReviews: Review[] = [
  {
    name: "علیرضا م.",
    tag: "High Volume Trader",
    rating: 5,
  ...
  <figure className={styles.card}>
    ...
    <figcaption className={styles.authorBox}>
      ...
      <div className={styles.authorInfo}>
        <h4 className={styles.name} dir="auto">
          {review.name}
        </h4>
        <span className={styles.volumeText} dir="ltr">
          {review.tag}
        </span>
      </div>
    </figcaption>
    ...
    async function fetchTestimonials() {
      try {
        ...
        const liveReviews: Review[] = testimonials.map((item, index) => {
          const firstName = item.profiles?.first_name || "کاربر";
          const lastNameInitial = item.profiles?.last_name
            ? ` ${item.profiles.last_name.charAt(0)}.`
            : "";

          return {
            name: `${firstName}${lastNameInitial}`,
            tag: traderTags[index % traderTags.length],
            text: item.message || "",
    ...
    <section className={styles.section} ref={sectionRef} aria-labelledby="test-title">
      <div className={styles.container}>
        <div className={styles.header}>
          ...
          <h2 id="test-title" className={styles.title}>
          ...
          <div className={styles.columnsContainer}>
            <div className={styles.column}>
              <div className={`${styles.track} ${styles.scrollUp}`}>
                {col1.map((review, i) => (
                  <ReviewCard key={`c1-a-${i}`} review={review} />
                ))}
              </div>
              <div className={`${styles.track} ${styles.scrollUp}`} aria-hidden="true">
                {col1.map((review, i) => (
                  <ReviewCard key={`c1-b-${i}`} review={review} />
                ))}
              </div>
            </div>
            ...
            <div className={styles.column}>
              <div className={`${styles.track} ${styles.scrollDown}`}>
                {col2.map((review, i) => (
                  <ReviewCard key={`c2-a-${i}`} review={review} />
                ))}
              </div>
              <div className={`${styles.track} ${styles.scrollDown}`} aria-hidden="true">
                {col2.map((review, i) => (
                  <ReviewCard key={`c2-b-${i}`} review={review} />
                ))}
              </div>
            </div>
            ...
            <div className={styles.column}>
              <div className={`${styles.track} ${styles.scrollUp}`}>
                {col3.map((review, i) => (
                  <ReviewCard key={`c3-a-${i}`} review={review} />
                ))}
              </div>
              <div className={`${styles.track} ${styles.scrollUp}`} aria-hidden="true">
                {col3.map((review, i) => (
                  <ReviewCard key={`c3-b-${i}`} review={review} />
                ))}
              </div>
            </div>
          </div>
