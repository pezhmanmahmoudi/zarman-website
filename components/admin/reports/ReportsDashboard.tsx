...
function KpiCard({ metric }: { metric: ReportKpi }) {
  ...
  return (
    <Link href={metric.drilldownHref} className={styles.kpiCard}>
      ...
      <strong title={formatMetric(metric)}>{formatMetric(metric)}</strong>
...
export function ReportsDashboard({ data }: { data: EnterpriseReportData }) {
  ...
  return (
    <div className={styles.reports}>
      ...
      {(data.refresh.isStale || message) && <div role="status" aria-live="polite" className={data.refresh.lastError || message?.startsWith("Error") ? styles.errorBanner : styles.staleBanner}><span>{message ?? "New ledger activity is waiting for a reporting refresh."}</span>{data.refresh.staleSince && <small>Stale since {new Date(data.refresh.staleSince).toLocaleString("en-AU")}</small>}</div>}
      {!dashboard.freshness && <div className={styles.emptyState}><h2>Your reports are ready to set up</h2><p>Build your first report to see your financial and operating activity.</p><button type="button" onClick={refresh} disabled={isPending}><RefreshCw size={16}/> Build reports</button></div>}
      ...
      <div className={styles.twoColumn}>
        ...
        <section className={styles.exportPanel}><div><span>Export Center</span><h2>Portable financial data</h2><p>Download the selected period with revenue, costs, profit, rates, inventory movement and cash flow.</p></div><button type="button" onClick={() => exportCsv(data)}><Download size={18}/> Download CSV</button><small>Includes daily financial and operating metrics for the selected period.</small></section>
      </div>
    </div>
  );
}