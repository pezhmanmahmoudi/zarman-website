...
export function RequestProgress({ request, events = [], locale, spotlight = false }: {
  ...
  return <section className={styles.journey} data-mood={journey.mood} data-spotlight={spotlight} aria-label={fa ? "مراحل حواله" : "Transfer progress"}>
    ...
    <div className={styles.transferStrip}><span>{fa ? "مجموع پرداخت" : "You send"}<bdi data-private-value>{requestMoney(request.quote.funding_total,request.quote.funding_currency,locale)}</bdi></span><span className={styles.stripArrow} aria-hidden="true">→</span><span>{fa ? "دریافتی گیرنده" : "Recipient gets"}<bdi data-private-value>{requestMoney(request.quote.recipient_amount,request.quote.recipient_currency,locale)}</bdi></span></div>
    <ol className={styles.rail} aria-label={fa ? "مراحل حواله" : "Transfer progress"}>
      {milestones.map((milestone,index)=><li key={milestone.key} className={styles.step} data-done={milestone.done} data-current={milestone.current} aria-current={milestone.current ? "step" : undefined}>
        ...
        <div><strong>{milestone.label[fa ? 1 : 0]}</strong>{milestone.at ? <time dir="ltr" dateTime={milestone.at}>{requestDate(milestone.at,locale)}</time> : <small>{milestone.current ? (fa ? "مرحله فعلی" : "Current step") : (fa ? "در انتظار" : "Up next")}</small>}</div>
      </li>)}
    </ol>
