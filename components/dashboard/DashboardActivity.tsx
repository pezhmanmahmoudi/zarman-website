  ...
  return <section aria-label={compact ? copy.recent : copy.history} className="min-w-0">
    <DashboardCard className="overflow-hidden p-0 sm:p-0">
      ...
      {loading && !visible.length ? <div className="px-5 py-9 text-sm text-[#626a76] sm:px-6" role="status">{copy.loading}</div> : <>
        {!!visible.length && <AnimatedList mode="live" motionEnabled={motionEnabled} role="list" aria-label={copy.history} className="gap-0 [&>div:last-child_a]:border-b-0">{visible.map(request => <TransferActivityRow key={request.id} request={request} locale={locale} createdDate />)}</AnimatedList>}
