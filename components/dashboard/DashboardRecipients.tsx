...
export function DashboardRecipients() {
  ...
  return <div className={styles.page}>
    <header className={styles.heading}><div><p>{text("YOUR CONNECTIONS","ارتباط‌های شما")}</p><h1>{text("People. Not paperwork.","آدم‌ها، نه کاغذبازی.")}</h1><span>{text("Saved accounts, ready for your next transfer.","حساب‌های ذخیره‌شده، آماده انتقال بعدی شما.")}</span></div><button className={styles.primary} onClick={()=>setModal("standard")}><Plus size={18}/>{text("New recipient","گیرنده جدید")}</button></header>
