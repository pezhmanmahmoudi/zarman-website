...
export function DashboardRateCard({ tailoredRate, loyaltySavings, baseRate, loyaltyBonus = 0, txType = "buy_aud" }: {
  ...
  return <DashboardMagicCard tone="violet" data-overview-card="rate">
    ...
    <div className="mt-5 text-[2.35rem] font-semibold leading-tight tracking-[-.04em] text-[#302346] tabular-nums"><bdi data-private-value>{tailoredRate ? number(tailoredRate) : "—"}</bdi> <span className="text-xs font-normal tracking-normal text-[#6a6078]">{fa ? "تومان / 1 AUD" : "Toman / 1 AUD"}</span></div>
  ...
  return <DashboardMagicCard tone="teal" motionEnabled={motionEnabled} contentClassName="p-0 sm:p-0" data-overview-card="loyalty">
    <div className="relative overflow-hidden p-6 pb-5 sm:p-7 sm:pb-5" style={{ background: "radial-gradient(ellipse at 100% 0%, #d5f2e9, transparent 75%)" }}>
      ...
      <div className="relative flex items-center justify-between gap-4">
        ...
        <div aria-hidden="true" className="relative flex size-16 shrink-0 items-center justify-center rounded-full border-2 border-[#bd9643] bg-[#fff0b9] text-[#8e5e13] shadow-[0_3px_0_0_#d9b364,inset_0_0_0_5px_#fff7dc]"><Trophy size={27} strokeWidth={1.6} /></div>
      </div>
      <div className="relative mt-7 flex flex-wrap items-end justify-between gap-3">
        <p className="m-0 text-[3.25rem] font-semibold leading-none tracking-[-.045em] text-[#194f43] tabular-nums" dir="ltr">{(pct * 100).toLocaleString("en-AU", { maximumFractionDigits: 2 })}<span className="ms-1 text-2xl">%</span></p>
...
export function DashboardOverview({ volume, completedCount, tailoredRate, baseRate, loyaltyBonus = 0, txType = "buy_aud" }: {
  ...
  return <div className="min-w-0 space-y-6 sm:space-y-7">
    <DashboardPageHeader eyebrow={copy.overview} title={<AuroraText className="max-w-full break-words [overflow-wrap:anywhere]" motionEnabled={motionEnabled} colors={["#6841b8", "#ac3978", "#30699d", "#287a70"]}>{fa ? "سلااام" : "Helloooo"}{customerName && <>, <bdi>{customerName}</bdi></>}<span aria-hidden="true">.</span></AuroraText>} description={copy.overviewHint} action={focus ? <DashboardButton asChild><Link href={dashboardHref(locale, "transfer")}>{copy.newTransfer}</Link></DashboardButton> : undefined} />
    ...
    <DashboardReveal motionEnabled={motionEnabled}>
      <BentoGrid className="grid-cols-2 gap-3 lg:grid-cols-4 sm:gap-4">
        <DashboardMagicCard tone="violet" motionEnabled={motionEnabled} contentClassName="p-4 sm:p-5" data-overview-card="volume"><p className="m-0 text-xs leading-5 text-[#645477]">{copy.volume}</p><p className="mb-0 mt-3 break-words text-2xl font-semibold text-[#3d2862] tabular-nums" data-private-value><bdi>{volume.toLocaleString("en-AU", { maximumFractionDigits: 2 })}</bdi> <span className="text-xs font-medium text-[#645477]">AUD</span></p></DashboardMagicCard>
        {metrics.map(({ label, value, tone, Icon }, index) => <DashboardMagicCard key={label} tone={tone} motionEnabled={motionEnabled} contentClassName="p-0 sm:p-0"><Link href={dashboardHref(locale, "history")} className="block h-full min-w-0 rounded-[inherit] p-4 no-underline outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#635bff]/60 sm:p-5"><span className="block text-xs leading-5" style={{ color: dashboardPalette[tone].ink }}>{label}</span><span className="mt-3 flex items-center justify-between gap-2"><strong className="block text-2xl font-semibold tabular-nums" style={{ color: dashboardPalette[tone].ink }}>{index > 0 && (feed.loading || feed.error) ? "—" : value.toLocaleString("en-AU")}</strong><Icon size={20} strokeWidth={1.7} style={{ color: dashboardPalette[tone].accent }} aria-hidden="true" /></span></Link></DashboardMagicCard>)}
      </BentoGrid>
    </DashboardReveal>
