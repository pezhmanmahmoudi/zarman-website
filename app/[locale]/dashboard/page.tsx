...
export default function ZarmanDashboard() {
  ...
  if (tab === "overview") return <DashboardOverview volume={volume} completedCount={completed.length} tailoredRate={tailoredRate}/>;
  ...
  if (tab === "transfer") return <div className={styles.page}>
    ...
    <ol className={styles.steps}><li><span>1</span>{copy.amounts}</li><li><span>2</span>{copy.details}</li><li><span>3</span>{copy.review}</li></ol>
    <div className={styles.transferLayout}><DashboardRequestHub isApproved={approved} txType={txType} setTxType={setTxType} amountStr={amountStr} setAmountStr={setAmountStr} loyaltyBonus={loyalty} tailoredRate={tailoredRate} baseRate={baseRate} profile={profile}/>
      <aside className={styles.transferAside}><DashboardRateCard tailoredRate={tailoredRate} loyaltySavings={Number(profile?.loyalty_discount_toman||0)}/><div className={styles.helpCard}><ShieldCheck size={24}/><h2>{locale === "fa" ? "هر مرحله، با اطلاع شما." : "Clarity at every step."}</h2><p>{locale === "fa" ? "مشخصات بانکی پس از تأیید درخواست نمایش داده می‌شود. رسید را همان‌جا ارسال و وضعیت انتقال را دنبال کنید." : "Bank details appear after approval. Upload your receipt and follow the transfer in your dashboard."}</p><Link className={styles.textLink} href={dashboardHref(locale,"history")}>{copy.messages}<ArrowUpRight size={14}/></Link></div></aside>
    </div>
  </div>;
